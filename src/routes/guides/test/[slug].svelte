...
<!-- {#if !filter}
	<button
		class="fixed top-44 right-0 w-12 h-12 rounded-l-lg bg-white dark:bg-gray-900 flex justify-center items-center shadow-xl z-50"
...
<div class="relative py-16 overflow-hidden">
	<div class="relative px-4 sm:px-6 lg:px-8">
		<article class="mt-6 prose md:prose-lg dark:prose-light mx-auto">
			<input type="checkbox" id="0" name="0" aria-label="level 0 filter" on:click={filterLevel} />
			<label for="0">Cantrip</label>
			<input type="checkbox" id="1" name="1" aria-label="level 1 filter" on:click={filterLevel} />
			<label for="1">1st</label>
			<input type="checkbox" id="2" name="2" aria-label="level 2 filter" on:click={filterLevel} />
